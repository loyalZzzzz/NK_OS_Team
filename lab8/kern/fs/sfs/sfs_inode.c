...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    *alenp = 0;
    ...
    uint32_t nblks = endpos / SFS_BLKSIZE - blkno;  // The number of complete blocks to Rd/Wr

    // LAB8:EXERCISE1 YOUR CODE
    
    // 使用 uint8_t 指针进行算术运算，避免 void* 运算警告
    uint8_t *data = (uint8_t *)buf; 

    // (1) If offset isn't aligned with the first block, Rd/Wr some content from offset to the end of the first block
    blkoff = offset % SFS_BLKSIZE;
    if (blkoff != 0) {
        // 如果 nblks == 0，说明读写范围在一个块内，大小为 endpos - offset
        // 否则，说明跨块，第一块读写到块末尾，大小为 SFS_BLKSIZE - blkoff
...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    ...
    *alenp = 0;
    ...
    // STEP 1: Handle unaligned start (Head)
    blkoff = offset % SFS_BLKSIZE;
    if (blkoff != 0) {
        size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
        ...
        alen += size;
        data += size;
        offset += size; // Advance offset to track position
        blkno++;        // Move to next block logical index
        nblks--;        // Decrement remaining full block count if we crossed a boundary
    }

    // STEP 2: Handle full aligned blocks (Body)
    // We recalculate nblks based on the updated offset to be safe, 
    // or just trust the decrement above.
    // Let's rely on current offset vs endpos for clarity.
    ...
    // STEP 3: Handle unaligned end (Tail)
    if (offset < endpos) {
        size = endpos - offset;
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &ino)) != 0) {
            goto out;
        }
        // Offset is now aligned to block start (0) because we handled Head and Body
        if ((ret = sfs_buf_op(sfs, data, size, ino, 0)) != 0) {
            goto out;
        }
        alen += size;
