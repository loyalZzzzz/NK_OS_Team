...
void proc_run(struct proc_struct *proc)
{
    ...
    // 加载新进程的页目录表（satp寄存器存储页表基地址）
    lsatp(proc->pgdir);
    // 刷新TLB（确保页表切换生效）
    flush_tlb();
    // 切换上下文：从prev的context到proc的context
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // LAB5: Update Step 5 (Use set_links)
    // Remove Lab 4 code: list_add(&proc_list, ...); nr_process++;
    hash_proc(proc);
...
load_icode(int fd, int argc, char **kargv) {
    ...
    for (int i = 0; i < elf->e_phnum; i++) {
        ...
        // 4.1 Load File Content (TEXT/DATA)
        ret = -E_NO_MEM;
        while (start < end) {
        ...
        // 4.2 Handle BSS (Zero out remaining memory)
        if (end < end_mem) {
            // Case 1: Zero out the remainder of the *current* page if we stopped in the middle
            size_t page_off = start & (PGSIZE - 1);
            if (page_off != 0) {
                // The page must exist because we just wrote data to it above.
                // However, if filesz was 0, it might not. But mm_map protects us usually.
                // To be safe, we retrieve the page.
            ...
            // Case 2: Allocate and zero full new pages for the rest of BSS
            while (start < end_mem) {
    ...
    // 6. Push Arguments
    stacktop = USTACKTOP;
    char *user_vaddr_argv[EXEC_MAX_ARG_NUM];
    ...
    for (int i = 0; i < argc; i++) {
        ...
        user_vaddr_argv[i] = (char *)stacktop;
    }
    
    // 6.2 Align Stack Pointer for argv array (RISC-V requires 16-byte alignment)
    stacktop = (stacktop - (argc + 1) * sizeof(uintptr_t)) & ~(0xF);
    uargv = (char **)stacktop;
    
    // 6.3 Write argv array
    for (int i = 0; i < argc; i++) {
