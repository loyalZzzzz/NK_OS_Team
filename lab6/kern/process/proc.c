...
load_icode(unsigned char *binary, size_t size)
{
     ...
     */
    // 1. Set user stack pointer
    tf->gpr.sp = USTACKTOP;
    // 2. Set entry point
    tf->epc = elf->e_entry;
    // 3. Set status register:
    //    - Clear SPP (Supervisor Previous Privilege) to 0 for User Mode
    //    - Set SPIE (Supervisor Previous Interrupt Enable) to 1 to enable interrupts
...
#else
    KERNEL_EXECVE(priority);//schedtest
#endif
